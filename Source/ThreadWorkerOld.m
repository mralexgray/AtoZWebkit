...
-(void)runPrimaryTask:(id)notUsed
{
	 ...
	 if( [[_target methodSignatureForSelector:_selector] numberOfArguments] == 4 ) // 2 hidden + 2 exposed
		  userInfo = [_target performSelectorWithoutWarnings:_selector withObject:_argument
											withObject:self];
	 else
		  userInfo = [_target performSelectorWithoutWarnings:_selector withObject:_argument];
	 ...
	 if( _didEndSelector )
		  [(id)[_callingConnection rootProxy] performSelectorWithoutWarnings:_didEndSelector withObject:userInfo];
